...
function getCovidDeaths() {
  const url = 'https://data.alexbilson.dev/medical_examiner_case_archive__covid19_related_deaths.json?sql=select%0D%0A++death_date%2C%0D%0A++age%2C%0D%0A++gender%2C%0D%0A++race%2C%0D%0A++primarycause%2C%0D%0A++incident_street%2C%0D%0A++incident_city%0D%0Afrom%0D%0A++medical_examiner_case_archive__covid19_related_deaths%0D%0Awhere%0D%0A++death_date+%3E%3D+date%28%27now%27%2C+%27-1+days%27%29%0D%0Aorder+by%0D%0A++death_date+desc';
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      data.rows.map(r => {
        ...
        var race = getRace(r[rowIndex.race]);
        var cause = getCause(r[rowIndex.primarycause]);
        var address = getAddress(r[rowIndex.incident_street], r[rowIndex.incident_city]);
